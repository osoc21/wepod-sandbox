...
testButton.onclick = async function () {
  console.log('session', session, session.info.isLoggedIn, session.info.webID)

  const RESOURCE = "https://jerrykakooza.inrupt.net/private/"
  const AGENT = "https://jerrykakooza.inrupt.net/profile/card#me"
  // see what access the public has to the resource
  await access.getPublicAccess(
    RESOURCE,   // Resource
    ...
    if (resourceAccess === null) {
      ...
      console.log("Public Access:: ", JSON.stringify(resourceAccess));
      console.log("Everyone", (resourceAccess.read ? 'CAN' : 'CANNOT'), "read the Resource.");
      console.log("Everyone", (resourceAccess.append ? 'CAN' : 'CANNOT'), "add data to the Resource.");
      console.log("Everyone", (resourceAccess.write ? 'CAN' : 'CANNOT'), "change data in the Resource.");
      console.log("Everyone", (resourceAccess.controlRead ? 'CAN' : 'CANNOT'), "see access to the Resource.");
      console.log("Everyone", (resourceAccess.controlWrite ? 'CAN' : 'CANNOT'), "change access to the Resource.");
    }
  })
  ...
  await access.getAgentAccess(
    RESOURCE,       // resource  
    AGENT,  // agent
    { fetch: session.fetch }                      // fetch function from authenticated session
  ).then(resourceAccess => {
    logAccessInfo("https://example.pod/profile#webId", resourceAccess, RESOURCE);
  });
  ...
  // The returned access can be an object { read: <boolean>, append: <boolean>, ... } 
  // or null if the access data is inaccessible to the user.
  await access.getAgentAccessAll(
    RESOURCE, // resource
    { fetch: session.fetch }                // fetch function from authenticated session
  ).then(accessByAgent => {
    // => accessByAgent is an object with Agent WebIDs as keys,
    //    and their associated access object {read: <boolean>, ... } as values.
    for (const [agent, resourceAccess] of Object.entries(accessByAgent)) {
      logAccessInfo(agent, resourceAccess, RESOURCE);
    }
  });
